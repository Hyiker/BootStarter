...
function start(){
    ...
    if [ $count != 0 ];then
        ...
        nohup java -jar $filename > /root/hyikercom/$ApplicationName.log 2>&1 &
    fi
    echo -e "\033[0;34m tail -10f hyikercom/hyikercom.log \033[0m"
}
...
function switch(){
    ...
    if [ "$1" != "" ];
    then
        using="/root/hyikercom/hyikercom-$1.jar"
    fi
    echo "正在启动 $using"
    nohup java -jar $using > /root/hyikercom/$ApplicationName.log 2>&1 & 
    echo "启动 $using 成功"
    echo -e "\033[0;34m tail -10f hyikercom/hyikercom.log \033[0m"
}
case $1 in
